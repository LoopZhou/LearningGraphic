...
import { RoomChildren } from './world/Room';
...
export default class Preloader extends EventEmitter {
  ...
  scaleFlag: boolean;
  moveFlag: boolean;
  initalY: number | null = null;
  scrollOnceEvent: (e: WheelEvent) => void;
  ...
  firstIntro() {
    return new Promise((resolve) => {
      ...
      timeline
        ...
        .to((this.room as THREE.Group).position, {
          x: -1,
          ease: 'power1.out',
          duration: 0.7,
        });
  ...
  onTouchMove(e: TouchEvent) {
    if (this.initalY) {
      ...
      const difference = this.initalY - currentY;
      if (difference > 0) {
  ...
  secondIntro() {
    return new Promise((resolve) => {
      ...
      secondTimeline
        ...
        .to(
          (this.room as THREE.Group).position,
          {
            x: 0,
            y: 0,
            z: 0,
  ...
  scale() {
    if (!this.room || !this.roomChildren) {
      return;
    }
