...
function color(_x: number, _y: number) {
  ...
  const res = trace2(world, r);

  return [res.e0, res.e1, res.e2];
}

// 获取了 hit 结果中反射光线，再带入 trace 递归
function trace(sence: Hitable, r: Ray, step = 0): Vec3 {
  // 加入了 step 参数，反射数量超过了50次，就直接返回黑色
  if (step > 50) {
...
// 大多数物体反射率不到 100%，所以我们需要一个参数设定物体的反射系数
function trace2(sence: Hitable, r: Ray, step = 0): Vec3 {
  if (step > 50) {
    return new Vec3(0, 0, 0);
  }
